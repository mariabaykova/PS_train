...
function intersectRanges(str1, str2) {
    ...
    const set1 = new Set(arr1.map((el) => [...el]).reduce((acc, cur) => [...acc, ...cur], []));
    const arr2 = str2.split(";").map((el) => {
        const [from, to,] = [...parceRange(el)];
        return unwrapRange(from, to);
    });
    const set2 = new Set(arr2.map((el) => [...el]).reduce((acc, cur) => [...acc, ...cur], []));
    // console.log(set1);
    // console.log(set2);
    return wrapIterable(intersect2Sets(set1, set2));
}
function parceRange(str) {
...
function wrapIterable(iterableIter) {
    const resArr = [];
